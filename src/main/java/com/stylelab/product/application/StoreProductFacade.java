...
public class StoreProductFacade {
    ...
    private void validationCreateStoreProductRequest(Long storeId, CreateStoreProductRequestVo createStoreProductRequestVo) {
        ...
        if (entryMain == null || !StringUtils.hasText(entryMain.entryMain())) {
            throw new ProductException(ProductError.PRODUCT_ENTRY_MAIN_REQUIRE, ProductError.PRODUCT_ENTRY_MAIN_REQUIRE.getMessage());
        }

        List<CreateStoreProductRequestVo.EntrySub> entrySubs = createStoreProductRequestVo.entrySubs();
        if (ObjectUtils.isEmpty(entrySubs)) {
            throw new ProductException(ProductError.PRODUCT_ENTRY_SUB_REQUIRE, ProductError.PRODUCT_ENTRY_SUB_REQUIRE.getMessage());
        }
        ...
        if (ObjectUtils.isEmpty(descriptions)) {
            throw new ProductException(ProductError.PRODUCT_DESCRIPTION_REQUIRE, ProductError.PRODUCT_DESCRIPTION_REQUIRE.getMessage());
        }
        ...
        if (productRequest == null) {
            throw new ProductException(ProductError.PRODUCT_REQUIRE, ProductError.PRODUCT_PRICE_REQUIRE.getMessage());
        }

        if (!StringUtils.hasText(productRequest.productCategoryPath())) {
            throw new ProductException(ProductError.PRODUCT_CATEGORY_PATH_REQUIRE, ProductError.PRODUCT_CATEGORY_PATH_REQUIRE.getMessage());
        }

        if (!StringUtils.hasText(productRequest.name())) {
            throw new ProductException(ProductError.PRODUCT_NAME_REQUIRE, ProductError.PRODUCT_NAME_REQUIRE.getMessage());
        }

        int price = productRequest.price();
        if (price < 3_000 || price > 1_000_000_000) {
            throw new ProductException(ProductError.PRODUCT_PRICE_OUT_OF_RANGE, ProductError.PRODUCT_PRICE_OUT_OF_RANGE.getMessage());
        }
        ...
        if (!useOption) {
            ...
            if (quantity < 0) {
                throw new ProductException(ProductError.PRODUCT_QUANTITY_LESS_THEN_ZERO, ProductError.PRODUCT_QUANTITY_LESS_THEN_ZERO.getMessage());
            }
        } else {
            int optionDepth = productRequest.optionDepth();

            if (optionDepth <= 0 || optionDepth > 2) {
                throw new ProductException(ProductError.PRODUCT_OPTION_DEPTH_OUT_OF_RANGE, ProductError.PRODUCT_OPTION_DEPTH_OUT_OF_RANGE.getMessage());
            }

            String option1 = productRequest.option1();
            if (!StringUtils.hasText(option1)) {
                throw new ProductException(ProductError.PRODUCT_OPTION1_NAME_REQUIRE, ProductError.PRODUCT_OPTION1_NAME_REQUIRE.getMessage());
            }

            if (optionDepth == 2) {
                String option2 = productRequest.option2();
                if (!StringUtils.hasText(option2)) {
                    throw new ProductException(ProductError.PRODUCT_OPTION2_NAME_REQUIRE, ProductError.PRODUCT_OPTION2_NAME_REQUIRE.getMessage());
                }
            }
            ...
            for (CreateStoreProductRequestVo.ProductOption1sRequest productOption1sRequest : productOption1sRequests) {
                ...
                if (!StringUtils.hasText(option1Name)) {
                    throw new ProductException(ProductError.OPTION1_NAME_REQUIRE, ProductError.OPTION1_NAME_REQUIRE.getMessage());
                }
                ...
                if (option1AdditionalPrice < 0 || option1AdditionalPrice > 100_000_000) {
                    throw new ProductException(ProductError.OPTION1_ADDITIONAL_PRICE_OUT_OF_RANGE, ProductError.OPTION1_ADDITIONAL_PRICE_OUT_OF_RANGE.getMessage());
                }

                if (optionDepth == 1) {
                    int option1Quantity = productOption1sRequest.quantity();
                    if (option1Quantity < 0) {
                        throw new ProductException(ProductError.OPTION1_EXISTS_PRODUCT_QUANTITY_GRATE_THEN_ZERO, ProductError.OPTION1_EXISTS_PRODUCT_QUANTITY_GRATE_THEN_ZERO.getMessage());
                    }
                } else {
                    List<CreateStoreProductRequestVo.ProductOption2sRequest> productOption2sRequests = productOption1sRequest.productOption2sRequest();
                    ...
                    for (CreateStoreProductRequestVo.ProductOption2sRequest productOption2sRequest : productOption2sRequests) {
                        ...
                        if (!StringUtils.hasText(option2Name)) {
                            throw new ProductException(ProductError.OPTION2_NAME_REQUIRE, ProductError.OPTION2_NAME_REQUIRE.getMessage());
                        }
                        ...
                        if(option2AdditionalPrice < 0 || option2AdditionalPrice > 100_000_000) {
                            throw new ProductException(ProductError.OPTION2_ADDITIONAL_PRICE_OUT_OF_RANGE, ProductError.OPTION2_ADDITIONAL_PRICE_OUT_OF_RANGE.getMessage());
                        }

                        int option2Quantity = productOption2sRequest.quantity();
                        if (option2Quantity < 0) {
                            throw new ProductException(ProductError.OPTION2_EXISTS_PRODUCT_QUANTITY_GRATE_THEN_ZERO, ProductError.OPTION2_EXISTS_PRODUCT_QUANTITY_GRATE_THEN_ZERO.getMessage());
                        }
                    }
                }
            }
